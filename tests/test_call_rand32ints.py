...
from function_app import rand32ints


class TestFunction(unittest.TestCase):
    def test_rand32ints_0(self):
        ...
        # Call the function.
        func_call = rand32ints.build().get_user_function()
        resp = func_call(req)
        body = resp.get_body()
        ...
        mys = f"1 unsigned 32 bit integer(s)"  # default is 1, we will not return 0 random ints.
    ...
    def test_rand32ints_10(self):
        ...
        # Check the output.
        self.assertIn("Random number(s):", body)
        mys = f"{count} unsigned 32 bit integer(s)"
        self.assertIn(mys, body)
        result_start = body.find("Random number(s):<br />") + len(
            "Random number(s):<br />"
        )
    ...
    def test_rand32ints_string(self):
        # Construct a mock HTTP request.
        count = 10
        req = func.HttpRequest(
            method="GET",
