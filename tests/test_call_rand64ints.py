...
class TestFunction(unittest.TestCase):
    ...
    def test_rand64ints_0(self):
        ...
        # Call the function.
        func_call = rand64ints.build().get_user_function()
        resp = func_call(req)
        body = resp.get_body()
        ...
        mys = f"1 unsigned 64 bit integer(s)"  # default is 1, we will not return 0 random ints.
    ...
    def test_rand64ints_10(self):
        ...
        # Check the output.
        self.assertIn("Random number(s):", body)
        mys = f"{count} unsigned 64 bit integer(s)"
        self.assertIn(mys, body)
        result_start = body.find("Random number(s):<br />") + len(
            "Random number(s):<br />"
        )
